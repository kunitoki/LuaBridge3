...
TEST_F(InspectTests, LuaTableVisitorHandlesNestedNamespaces)
{
    struct OuterCls { void outerMethod() {} };
    struct InnerCls { void innerMethod() {} };

    luabridge::getGlobalNamespace(L)
        .beginNamespace("Outer")
            .beginClass<OuterCls>("OuterCls")
                .addFunction("outerMethod", &OuterCls::outerMethod)
            .endClass()
            .beginNamespace("Inner")
                .beginClass<InnerCls>("InnerCls")
                    .addFunction("innerMethod", &InnerCls::innerMethod)
                .endClass()
            .endNamespace()
        .endNamespace();

    int topBefore = lua_gettop(L);
    luabridge::inspectToLua(L, "Outer");

    // Stack must grow by exactly 1
    ASSERT_EQ(topBefore + 1, lua_gettop(L));
    ASSERT_TRUE(lua_istable(L, -1));

    // Outer namespace name
    lua_getfield(L, -1, "name");
    EXPECT_STREQ("Outer", lua_tostring(L, -1));
    lua_pop(L, 1);

    // Outer classes array contains OuterCls
    lua_getfield(L, -1, "classes");
    EXPECT_TRUE(lua_istable(L, -1));
    EXPECT_EQ(1, luabridge::get_length(L, -1));
    lua_pop(L, 1);

    // subNamespaces array contains the Inner namespace
    lua_getfield(L, -1, "subNamespaces");
    EXPECT_TRUE(lua_istable(L, -1));
    EXPECT_EQ(1, luabridge::get_length(L, -1));

    // Inspect the Inner sub-namespace table
    lua_rawgeti(L, -1, 1);
    ASSERT_TRUE(lua_istable(L, -1));

    lua_getfield(L, -1, "name");
    EXPECT_STREQ("Inner", lua_tostring(L, -1));
    lua_pop(L, 1);
    ...
    lua_pop(L, 1); // pop subNamespaces table
